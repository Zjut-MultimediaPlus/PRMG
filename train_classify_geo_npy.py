...
def train_model(
    ...
    savedir = './checkpoints_cls/' + start_time + '/'
    ...
    for epoch in range(1, epochs + 1):
            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.n_classes == 1:
                        ...
                        loss = criterion(masks_pred.squeeze(1)[~mask_index], true_masks.float()[~mask_index])

                    else:
                        mask_index = torch.isnan(true_masks)

                        mask_pred_flatten = masks_pred.permute(1, 0, 2, 3)[:, ~mask_index].T.float()

                        loss = criterion(mask_pred_flatten, true_masks[~mask_index].long())
                ...
                # Evaluation round
            division_step = 20
            if epoch % division_step == 0 or epoch == epochs:
                val_score, val_str, val_list = evaluate(model, val_loader, device, amp)
        ...
        if save_checkpoint and epoch % 20 == 0:
            ...
            test_score, test_str, test_list = evaluate(model, test_loader, device, amp)
...
if __name__ == '__main__':
    ...
    # train_dir_img = Path('E:/2021_2022_dataset/PRMD_DATA_21-22_npy/train')
    ...
    #
    # train_dir_img = Path('E:/2021_2022_dataset/PRMD_DATA_21-22_npy_v3/train')
    # val_dir_img = Path('E:/2021_2022_dataset/PRMD_DATA_21-22_npy_v3/val')
    # test_dir_img = Path('E:/2021_2022_dataset/PRMD_DATA_21-22_npy_v3/test')
    # #
    ...
    # train_dir_img = Path('/opt/data/private/PRMD_DATA_21-22_npy_v2/train')
    # val_dir_img = Path('/opt/data/private/PRMD_DATA_21-22_npy_v2/val')
    # test_dir_img = Path('/opt/data/private/PRMD_DATA_21-22_npy_v2/test')
